...
namespace DataLayerService
{
    public class MemberSvcImpl : IMemberSvc
    {
        ...
        public void addMember(member mem)
        {
            ...
            catch (DbUpdateException e) //DbContext
            {
                //Console.WriteLine(ex.InnerException);
                throw new ArgumentException("The username is already exist!", e);
            }
        ...
        public bool GetAdminUser()
        {
            bool isValid = false;
            using (var db = new PointAppDBContext())
            {
                var dbList = db.members.SingleOrDefault(u => u.username == "adminuser");
                if (dbList != null)
                {
                    isValid = true;
                }
        ...
        public member GetById(string nuser)
        {
            return db.members.Where(p => p.username == nuser).SingleOrDefault() ;
        }
